...
<div class="container-fluid h-100">
    <div class="row h-100">
        ...
        <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4 main-content">
            @include('vendor.layouts.header', ['pageTitle' => 'Edit Banner'])
                        ...
                        <div class="card border-0 shadow-sm">
                            <div class="card-header bg-white border-0 py-3">
                                <div class="d-flex justify-content-between align-items-center">
                                    <div>
                                        <h4 class="card-title mb-0 fw-bold h5 h4-md">Edit Banner</h4>
                                        <p class="mb-0 text-muted small">Update banner details and image</p>
                                    </div>
                            ...
                            <div class="card-body">
                                ...
                                <form action="{{ route('vendor.banners.update', $banner->id) }}" method="POST" enctype="multipart/form-data">
                                    @csrf
                                    @method('PUT')
                                        ...
                                        <div class="col-md-8">
                                            ...
                                            <div class="mb-4">
                                                ...
                                                <div class="btn-group w-100" role="group">
                                                    <input type="radio" class="btn-check" name="link_type" id="link_type_category" value="category">
                                            ...
                                            <div class="mb-4">
                                                <label for="image" class="form-label fw-semibold">Replace Banner Image <span class="text-muted">(Optional)</span></label>
                                                <input type="file" 
                                                ...
                                                @error('image')
                                                    <div class="invalid-feedback">{{ $message }}</div>
                                                @enderror
                                                <small class="text-muted">Leave empty to keep current image. Accepted formats: JPEG, PNG, GIF, WEBP. Max size: 2MB</small>
                                            </div>
                                                ...
                                                <div class="card-body">
                                                    ...
                                                    <div class="mb-0">
                                                        <small class="text-muted">
                                                            ...
                                                            <strong>Updated:</strong> {{ $banner->updated_at->format('M d, Y') }}
                                                        </small>
                                                    </div>
                                                </div>
                                            </div>
...
<script>
    ...
    // Detect link type from current URL
    function detectLinkType() {
        if (!currentUrl) {
            // Default to category if no URL
            document.getElementById('link_type_category').checked = true;
        ...
        // Check if it's a category URL (format: /customer/categories/{id}/subcategories)
        const categoryMatch = currentUrl.match(/\/customer\/categories\/(\d+)\/subcategories/);
        if (categoryMatch) {
            ...
            if (category) {
                ...
                document.getElementById('url_hint').textContent = 'URL will be auto-generated based on category selection';
                console.log('Detected category link type, category ID:', categoryId);
                return;
            }
        }

        // Check if it's a product URL (format: /customer/products/{id})
        const productMatch = currentUrl.match(/\/customer\/products\/(\d+)/);
        if (productMatch) {
            const productId = productMatch[1];
            const product = productsData.find(p => p.id == productId);
            if (product) {
                document.getElementById('link_type_product').checked = true;
                ...
                document.getElementById('url_hint').textContent = 'URL will be auto-generated based on product selection';
                console.log('Detected product link type, product ID:', productId);
                return;
            }
        }

        // Default to category if no match found
        document.getElementById('link_type_category').checked = true;
        document.getElementById('category_selection').style.display = 'block';
        document.getElementById('product_selection').style.display = 'none';
    }
