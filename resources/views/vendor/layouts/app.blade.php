...
<head>
    ...
    <title>{{ setting('site_title', 'Vendor Panel') }} - @yield('title', 'Dashboard')</title>
...
<body class="bg-background text-text">
    ...
    <!-- CSRF Token Refresh and AJAX Setup -->
    <script>
        ...
        // Refresh CSRF token every 10 minutes to prevent session expiry issues
        ...
        $(document).ajaxError(function(event, jqXHR, settings, thrownError) {
            if (jqXHR.status === 419) {
                // CSRF token mismatch - refresh token and show message
                $.get('{{ route("csrf.refresh") }}').done(function(data) {
                    $('meta[name="csrf-token"]').attr('content', data.token);
                    $('input[name="_token"]').val(data.token);
                    $.ajaxSetup({
